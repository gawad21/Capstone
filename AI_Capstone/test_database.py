...
def update_status_flags(inDistress, isDrowning, LeafPercent):
    ...
    statRef.update({u'LeafPercent':LeafPercent})
...
def update_status_flags_alarm(alarmON):
    # Reference to document containing status flags
    statRef = db.collection(u'user1').document(u'status-flags')

    # For updating the three status flags
    statRef.update({u'alarmON': alarmON})
